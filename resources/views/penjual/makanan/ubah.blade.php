...
<div class="max-w-2xl mx-auto mt-10 bg-white p-6 rounded shadow">
    <h2 class="text-2xl font-bold mb-4">Edit Makanan</h2>
    ...
    <form action="{{ route('penjual.makanan.update', $food->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="number" name="estimated_time" value="{{ old('estimated_time', $food->estimated_time) }}" 
        ...
        <div class="mb-4">
            ...
            <select name="location" class="w-full border p-2 rounded @error('location') border-red-500 @enderror" required>
                @foreach($locations as $location)
                    <option value="{{ $location }}" {{ old('location', $food->location) == $location ? 'selected' : '' }}>
                        {{ $location }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="status" class="w-full border p-2 rounded @error('status') border-red-500 @enderror">
                <option value="Ready Stok" {{ old('status', $food->status) == 'Ready Stok' ? 'selected' : '' }}>Ready Stok</option>
                <option value="Stok Empty" {{ old('status', $food->status) == 'Stok Empty' ? 'selected' : '' }}>Stok Empty</option>
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            @if($food->photo_url)
                <img src="{{ asset($food->photo_url) }}" alt="Current Photo" class="w-32 h-32 object-cover mb-2">
            @endif
            <input type="file" name="photo" accept="image/*" 
                class="w-full border p-2 rounded @error('photo') border-red-500 @enderror">
            <p class="text-sm text-gray-600 mt-1">Kosongkan jika tidak ingin mengubah foto</p>
        </div>
