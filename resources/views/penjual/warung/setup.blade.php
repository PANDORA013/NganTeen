            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="alert alert-info">
                        ...
                        <p class="mb-0">Untuk dapat mengelola menu, pesanan, dan pencairan, Anda harus mendaftarkan warung terlebih dahulu. Silakan lengkapi data warung Anda di bawah ini.</p>
                    </div>

                    <form action="{{ route('penjual.warung.store') }}" method="POST" id="warungForm">
                        @csrf
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('deskripsi') is-invalid @enderror" 
                                              ...
                                              name="deskripsi" 
                                              rows="3" 
                                              placeholder="Ceritakan tentang warung Anda...">{{ old('deskripsi') }}</textarea>
                                    @error('deskripsi')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('rekening_bank') is-invalid @enderror" 
                                        ...
                                        <option value="BCA" {{ old('rekening_bank') == 'BCA' ? 'selected' : '' }}>BCA</option>
                                        <option value="BNI" {{ old('rekening_bank') == 'BNI' ? 'selected' : '' }}>BNI</option>
                                        <option value="BRI" {{ old('rekening_bank') == 'BRI' ? 'selected' : '' }}>BRI</option>
                                        <option value="Mandiri" {{ old('rekening_bank') == 'Mandiri' ? 'selected' : '' }}>Mandiri</option>
                                        <option value="CIMB Niaga" {{ old('rekening_bank') == 'CIMB Niaga' ? 'selected' : '' }}>CIMB Niaga</option>
                                        <option value="Danamon" {{ old('rekening_bank') == 'Danamon' ? 'selected' : '' }}>Danamon</option>
                                        <option value="BTN" {{ old('rekening_bank') == 'BTN' ? 'selected' : '' }}>BTN</option>
                                        <option value="Permata" {{ old('rekening_bank') == 'Permata' ? 'selected' : '' }}>Permata</option>
                                    </select>
                                    @error('rekening_bank')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-1"></i>Daftarkan Warung
                            </button>
                        </div>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Form validation
    const form = document.getElementById('warungForm');
