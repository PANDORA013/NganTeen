

<?php $__env->startSection('content'); ?>
<div class="container max-w-4xl mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-lg p-6">
        <h2 class="text-2xl font-bold text-gray-800 mb-6 flex items-center">
            <span class="text-2xl mr-2">🛒</span> Keranjang Belanja
        </h2>

        <?php if($keranjang->count() > 0): ?>
            <div class="space-y-4">
                <?php $__currentLoopData = $keranjang; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
                        <div class="flex items-center flex-1">
                            <div class="ml-4">
                                <h3 class="font-semibold text-gray-800"><?php echo e($item->menu->nama_menu); ?></h3>
                                <p class="text-sm text-gray-600"><?php echo e($item->menu->nama_warung); ?> - <?php echo e($item->menu->area_kampus); ?></p>
                                <p class="text-orange-500 font-medium">Rp <?php echo e(number_format($item->menu->harga, 0, ',', '.')); ?></p>
                            </div>
                        </div>

                        <div class="flex items-center gap-4">
                            <form action="<?php echo e(route('cart.update', $item->id)); ?>" method="POST" class="flex items-center">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('PUT'); ?>
                                <div class="flex items-center border border-gray-300 rounded-lg">
                                    <button type="button" onclick="decrementQuantity(this)" 
                                            class="px-3 py-1 text-gray-600 hover:bg-gray-100 rounded-l-lg">
                                        -
                                    </button>
                                    <input type="number" name="jumlah" value="<?php echo e($item->jumlah); ?>" min="1" max="<?php echo e($item->menu->stok); ?>"
                                           class="w-16 text-center border-x border-gray-300 py-1"
                                           onchange="this.form.submit()">
                                    <button type="button" onclick="incrementQuantity(this)"
                                            class="px-3 py-1 text-gray-600 hover:bg-gray-100 rounded-r-lg">
                                        +
                                    </button>
                                </div>
                            </form>

                            <div class="text-right min-w-[100px]">
                                <p class="font-semibold text-gray-800">
                                    Rp <?php echo e(number_format($item->menu->harga * $item->jumlah, 0, ',', '.')); ?>

                                </p>
                                <p class="text-sm text-gray-500">
                                    Stok: <?php echo e($item->menu->stok); ?>

                                </p>
                            </div>

                            <form action="<?php echo e(route('cart.destroy', $item->id)); ?>" method="POST" class="ml-4">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('DELETE'); ?>
                                <button type="submit" class="text-red-500 hover:text-red-700"
                                        onclick="return confirm('Yakin ingin menghapus item ini?')">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
                            </form>
                        </div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

                <div class="mt-6 border-t border-gray-200 pt-6">
                    <div class="flex justify-between items-center mb-6">
                        <span class="text-lg font-semibold text-gray-800">Total:</span>
                        <span class="text-2xl font-bold text-orange-500">
                            Rp <?php echo e(number_format($total, 0, ',', '.')); ?>

                        </span>
                    </div>

                    <form action="<?php echo e(route('checkout.process')); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                        <button type="submit" class="w-full bg-orange-500 text-white py-3 px-6 rounded-lg font-semibold hover:bg-orange-600 transition-colors">
                            Checkout
                        </button>
                    </form>
                </div>
            </div>
        <?php else: ?>
            <div class="text-center py-12">
                <div class="text-5xl mb-4">🛒</div>
                <h3 class="text-xl font-medium text-gray-600 mb-2">Keranjang Kosong</h3>
                <p class="text-gray-500 mb-6">Belum ada item yang ditambahkan ke keranjang</p>                <a href="<?php echo e(route('pembeli.menu.index')); ?>" class="inline-block bg-orange-500 text-white py-2 px-6 rounded-lg font-semibold hover:bg-orange-600 transition-colors">
                    Lihat Menu
                </a>
            </div>
        <?php endif; ?>
    </div>
</div>

<?php $__env->startPush('scripts'); ?>
<script>
function incrementQuantity(button) {
    const input = button.parentNode.querySelector('input');
    const max = parseInt(input.getAttribute('max'));
    const currentValue = parseInt(input.value);
    if (currentValue < max) {
        input.value = currentValue + 1;
        input.form.submit();
    }
}

function decrementQuantity(button) {
    const input = button.parentNode.querySelector('input');
    const currentValue = parseInt(input.value);
    if (currentValue > 1) {
        input.value = currentValue - 1;
        input.form.submit();
    }
}
</script>
<?php $__env->stopPush(); ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\NganTeen\resources\views\pembeli\keranjang.blade.php ENDPATH**/ ?>